(() => {
  ...
  function extractArticleText() {
    ...
    try {
      ...
      var article = new import_readability.Readability(clone).parse();
      if (article && article.textContent && article.textContent.trim().length > 80) {
        title = article.title || title;
        text = article.textContent.trim();
        return { title, text, source: "readability" };
      }
    } catch (e) {
    }
    var el = document.querySelector("article") || document.querySelector("main") || document.querySelector('[role="main"]');
    if (el && el.innerText && el.innerText.trim().length > 80) {
      return { title, text: el.innerText.trim(), source: "main-article" };
    }
    var body = document.body;
    if (body && body.innerText) {
      text = body.innerText.trim();
      if (text.length > 5e5) text = text.slice(0, 5e5);
      return { title, text, source: "body" };
    }
  ...
  function run() {
    ...
    var ext = typeof browser !== "undefined" && browser.runtime && browser.runtime.sendMessage ? browser : typeof chrome !== "undefined" && chrome.runtime && chrome.runtime.sendMessage ? chrome : null;
    if (!ext) {
